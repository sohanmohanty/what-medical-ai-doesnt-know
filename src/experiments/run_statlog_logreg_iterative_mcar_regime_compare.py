...
def main():
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits):
        ...
        clean_results = evaluate_pipeline(clean_pipeline, X_train_clean, X_test_clean, y_train, y_test)
        ...
        for rate in rates:
            ...
            X_test_corrupt, _, test_meta = apply_mcar(X_test_clean, rate=rate, rng=rng_test)

            for regime in regimes:
                if regime == "clean_train_corrupt_test":
                    X_train_used = X_train_clean.copy()
                    train_actual_rate = 0.0
                elif regime == "corrupt_train_corrupt_test":
                    rng_train = np.random.default_rng(42 + fold_idx)
                    X_train_used, _, train_meta = apply_mcar(X_train_clean, rate=rate, rng=rng_train)
                    train_actual_rate = train_meta["actual_rate"]
                else:
                    raise ValueError(f"Unknown regime: {regime}")

                pipeline = build_pipeline()
                results = evaluate_pipeline(pipeline, X_train_used, X_test_corrupt, y_train, y_test)
                ...
                row = {
                    ...
                    "fold": fold_idx,
                    "rate": rate,
                    "train_actual_rate": train_actual_rate,
    ...
    summary_df = (
        results_df.groupby(
            ["dataset", "model", "imputer", "mechanism", "regime", "rate"],
            as_index=False
        )
        .agg(
    ...
    results_df.to_csv("results/metrics/statlog_logreg_iterative_mcar_regime_compare_metrics.csv", index=False)
    summary_df.to_csv("results/metrics/statlog_logreg_iterative_mcar_regime_compare_summary.csv", index=False)

    print("\nSaved:")
    print("- results/metrics/statlog_logreg_iterative_mcar_regime_compare_metrics.csv")
    print("- results/metrics/statlog_logreg_iterative_mcar_regime_compare_summary.csv")
