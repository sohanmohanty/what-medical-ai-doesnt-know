...
class MissingIndicatorAppender(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        flags.columns = [f"{c}_missing" for c in X.columns]
...
def build_pipeline(variant):
    ...
    if variant == "simple_plus_indicators":
        steps.append(("append_indicators", MissingIndicatorAppender()))
    steps.extend([
...
def main():
    ...
    target_features = ["mean texture", "mean perimeter", "mean area", "worst texture", "worst perimeter"]
    all_results = []
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits):
        ...
        clean_results = evaluate_pipeline(build_pipeline("simple"), X_train, X_test, y_train, y_test)
        ...
        for rate in rates:
            ...
            for variant in variants:
                results = evaluate_pipeline(build_pipeline(variant), X_train_masked, X_test_masked, y_train, y_test)
                all_results.append({
                    "dataset": "wdbc",
    ...
    summary_df = results_df.groupby(["dataset", "model", "mechanism", "variant", "rate"], as_index=False).agg(
    ...
    results_df.to_csv("results/metrics/wdbc_logreg_mnar_indicator_compare_metrics.csv", index=False)
    summary_df.to_csv("results/metrics/wdbc_logreg_mnar_indicator_compare_summary.csv", index=False)

    print("Saved:")
    print("- results/metrics/wdbc_logreg_mnar_indicator_compare_metrics.csv")
    print("- results/metrics/wdbc_logreg_mnar_indicator_compare_summary.csv")
