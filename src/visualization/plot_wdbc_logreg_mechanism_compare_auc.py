...
def main():
    ...
    mnar_path = "results/metrics/wdbc_logreg_mnar_5fold_summary.csv"
    output_path = "figures/wdbc_logreg_mechanism_compare_auc.png"

    mcar_df = pd.read_csv(mcar_path)
    mar_df = pd.read_csv(mar_path)
    mnar_df = pd.read_csv(mnar_path)
    ...
    plt.title("WDBC Logistic Regression: Mean ROC-AUC by Mechanism")
    plt.legend()
    plt.tight_layout()
